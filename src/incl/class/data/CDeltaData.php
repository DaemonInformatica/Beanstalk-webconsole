...
class CDeltaData extends CData
{
  ...
  */
  private function updateReferences($value, $oldValue, $updatedBy)
  {
    ...
    $sql          = "UPDATE :tblName SET :colName=:value, updated=:updated, updatedBy=:updatedBy WHERE :oldColName=:oldValue;";
    $qry          = $this->getConnection()->prepare($sql);

    $qry->bindValue("tblName",    $tblName , PDO::PARAM_);
    $qry->bindValue("updated",    $updated, PDO::PARAM_);
    $qry->bindValue("updatedBy",  $updatedBy, PDO::PARAM_);
    ...
    for($i = 0; $i < $lenTableRef; $i++)
    {
      ...
      $qry->bindValue("colName",    $colName,   PDO::PARAM_STR);
      $qry->bindValue("value",      $value,     PDO::PARAM_STR);
      $qry->bindValue("oldColName", $colName,   PDO::PARAM_STR);
      $qry->bindValue("oldValue",   $oldValue,  PDO::PARAM_STR);
