...
class CPageMyMD5 extends CSecurePage
{
  ...
  public function createAddNewMD5String()
  {
    ...
    $code .= "  Start at: <input type=\"\" name=\"txt_start_value\" />\n";
  ...
  public function createMD5Overview()
  {
    ...
    */
    ...
    // show overview.
    $code .= "<table border=\"1\">\n";
    $code .= "  <tr><td>ID:</td><td>MD5:</td><td>Start at:</td><td>Decoded</td><td>status:</td><td>time spent:</td><td>created:</td><td>options:</td></tr>\n";

    foreach($arrMD5 as $pMD5)
    {
      $id       = $pMD5->getValue("id");
      $md5      = $pMD5->getValue("md5string");
      $start    = $pMD5->getValue("start");
  ...
  protected function handle_new_md5()
  {
    ...
    $startAt  = $_POST['txt_start_value'];
    $userID   = $this->getUserID();

    return $this->m_pLogicMyMD5->addNewMD5($strMD5, $startAt, $userID);
  }
}
?>