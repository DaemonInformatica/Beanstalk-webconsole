...
class CDataLanguage extends CData
{
  ...
  */
  public function getLanguageByName($strName)
  {
    ...
    $qry->bindValue("strName", $strName, PDO::PARAM_STR);
    $qry->execute();
  ...
  */
  ...
  public function loadIDByAbbr($abbr)
  {
    $sql = "SELECT id FROM tblLanguage WHERE abbreviation=:abbr";
    $qry = $this->getConnection()->prepare($sql);
    
    $qry->bindValue("abbr", $abbr, PDO::PARAM_STR);
    $qry->execute();
    ...
    if($row = $qry->fetch())
    {
      ...
      $this->setValue("abbreviation", $abbr,  true);
