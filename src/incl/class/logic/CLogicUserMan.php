...
include_once("incl/class/data/CDataUserToAccess.php");
...
class CLogicUserMan extends CLogic
{
  ...
  {
    print("CLogicUserMan::storeNewUser: userGroup: $userGroup<br />\n");
    ...
    print("LogicUserMan::storeNewUser: strUserGroup: $strUserGroup<br />\n");
    ...
    if($bResult)
    {
      ...
      for($i = 0; $i < $lenAccessElements; $i++)
      {
        ...
        $pUserAccess  = new CDataUserToAccess(0, false, $this->m_envs);
  ...
  public function storeNewUserGroup($userID, $groupID, $updatedBy)
  {
    ...
    foreach($arrElements as $pElement)
    {
  ...
  public function setUserAccessElement($userID, $elementID, $bConnect, $createdBy)
  {
    ...
    if(!$bConnect)
    {
      ...
      $pUserToAccess  = new CDataUserToAccess(0, false, $this->m_envs);
  ...
  public function getUserAccessElements($userID, $bInUser = true)
  {
    ...
    for($i = 0; $i < $lenElements; $i++)
    {
      ...
      if($pUser->hasAccessTo($strElement) == true && $bInUser == true)
        array_push($arr, $pElement);
       elseif($pUser->hasAccessTo($strElement) == false && $bInUser == false)
        array_push($arr, $pElement);
    }
